...
local function startLegitDig()
    ...
    if Runtime.QteAutoClickConn then Runtime.QteAutoClickConn:Disconnect() end
...
local function startMythicDig()
    ...
    if Runtime.MythicDigConn then Runtime.MythicDigConn:Disconnect() end
...
-- ============================================================
-- AUTO DEBRIS (FIXED VERSION)
-- ============================================================
local function startAutoDebris()
    if Runtime.DebrisRunning then return end
        ...
        while Settings.Main.AutoDebris do
            ...
            if #debrisList > 0 then
                ...
                while #debrisList > 0 do
                    if not Settings.Main.AutoDebris then
                        print("[Auto Debris] Toggle dimatikan, hentikan proses")
                        Runtime.DebrisActive = false
                        Runtime.DebrisRunning = false
                        return
                    end
                ...
                -- Cek apakah masih ada debris di workspace (termasuk yang tidak punya posisi)
                ...
                if not anyRemaining then
                    if Runtime.DebrisReturnPos then
                        ...
                        print("[Auto Debris] Semua debris selesai, kembali ke posisi awal")
                    end
                    Runtime.DebrisReturnPos = nil
                    Runtime.DebrisActive = false
                    Runtime.CompletedDebris = {}
                else
                    print("[Auto Debris] Masih ada debris tersisa (mungkin tanpa posisi), lanjut...")
                end
            else
                if Runtime.DebrisActive then
                    local anyDebris = false
                    for _, v in ipairs(workspace:GetChildren()) do
                        if v.Name:find("Debris") or v.Name:find("Impact") then
                            anyDebris = true
                            break
                        end
                    end
                    if not anyDebris then
                        if Runtime.DebrisReturnPos then
                            tpTo(Runtime.DebrisReturnPos)
                            print("[Auto Debris] Tidak ada debris, kembali ke posisi awal")
                        end
                        Runtime.DebrisReturnPos = nil
                        Runtime.DebrisActive = false
                        Runtime.CompletedDebris = {}
            ...
            pcall(function()
                for _, gui in pairs(LocalPlayer.PlayerGui:GetDescendants()) do
                    if gui:IsA("TextLabel") and tostring(gui.Text):match("^%d+/%d+$") then
                        ...
                        if tonumber(current) and tonumber(max) and current >= max then
                            inventoryFull = true
                            break
                        end
                    end
...
Tabs.Favorites:CreateButton({
    ...
    Callback = function()
        ...
        Fluent:Notify({ Title = "Unfavorite Rarity", Content = "Unfavorited " .. unfavCount .. " shells with selected rarities.", Duration = 3 })
    end
})
